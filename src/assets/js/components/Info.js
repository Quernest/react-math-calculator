import React, { Component } from 'react';

const Warning = () => {
    let iconInfo = "data:image/svg+xml;utf8;base64,";
    let h3   = "Information";
        
    return (
        <div id="warning">
            <div id="warning-title">
                <img src={iconInfo} alt="icon"/>
                <h3>{h3}</h3>
            </div>
            <div id="warning-content">
                <p>Исследуемая функция <i><strong>f(x)</strong></i> должна быть <strong>унимодальной</strong>.</p>
                <h4>Определение:</h4>
                <ul>
                    <li>Точка <strong>x*</strong> локального минимума функции принадлежит отрезку <strong>[a, b]</strong></li>
                    <li>Для любых двух точек отрезка <i>x1</i> и <i>x2</i>, взятых по одну сторону от точки минимума, <br/>
                        точке <i>x1</i> более близкой к точке минимума соответствует меньшее значение функции, <br/>
                        т.е. при <strong>x*</strong> &lt; <i>x1</i> &lt; <i>x2</i> либо при <i>x2</i> &lt; <i>x1</i> &lt; <strong>x*</strong> справедливо неравенство <i>f(x1)</i> &lt; <i>f(x2).</i> 
                    </li>
                </ul>
                <h4>Пример функций для вычисления:</h4>
                <ul>
                    <li>cos(x) на интервале [-2, 3]</li>
                    <li>x^2+2x на интервале [-3, 5]</li>
                    <li>sin(x)+x^2 на интервале [0, 1]</li>
                </ul>
                <h4>В случае исследования функции на большом интервале например (-20, 20) возможны некорректные результаты. Тогда необходимо уменьшить интервал поиска и попробовать снова.</h4>
            </div>
        </div>
    );
}

export default Warning;